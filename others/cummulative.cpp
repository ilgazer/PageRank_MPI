...
static std::vector<int> senders;
static std::vector<int> receivers;
...
void send_init_data_as_master(char *edge_file, char *metis_file)
{
    int src;
    int dest;
    int *sdata;
    ...
    std::ifstream edges(edge_file);
    std::ifstream metis_threads(metis_file);
...
int main(int argc, char *argv[])
{
    ...
    if (mypid == 0)
    {
        send_init_data_as_master(argv[1], argv[2]);
    }
    else
    {
        recv_init_data();
