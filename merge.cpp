...
double calculate_ranks()
{
  ...
  for (Node *node : owned_nodes)
  {
    ...
    // * ((*curr_ranks)[node->id] * (node->num_outgoing_edges + 1))
    ;
    sigma += std::abs((node->num_outgoing_edges + 1) * (*curr_ranks)[node->id] - rank);
    (*next_ranks)[node->id] = rank / (node->num_outgoing_edges + 1);
...
void init_ranks()
{
  ...
  {
    (*next_ranks)[node->id] = 1.0 / (node->num_outgoing_edges+ 1 )
    // / 1850065
    ;
